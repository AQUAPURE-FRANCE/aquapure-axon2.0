...
abstract class HsMultiAccessoriesAbstract extends HsMaModule
{
     ...
     * @return bool
     */
    protected abstract function isHookedInCurrentPage();
    ...
    protected abstract function assignSmartyVariables();
...
//        return $this->renderBlockAccessories($id_products, false);
     ...
     * @return html
     */
    protected abstract function renderBlockAccessories($id_products, $is_product_page = true);
     ...
     * @return HTML string
     */
    public abstract function hookDisplayRightColumnProduct($params = null);
                ...
                foreach ($this->accessories as $id_accessory) {
                    ...
                    if ($accessory_qty[$i] > 0) {
                        ...
                        if ($is_added) {
                            ...
                            $cart_product = new HsAccessoryCartProductAbstract($this->context->cart->id, $id_product, $id_main_product_attribute, $id_accessory, $id_product_attribute, $id_accessory_customization);
                    ...
                    if ($total_product_incart > 0 && $current_product_quantity_incart < $total_product_incart) {
                        ...
                        if (!empty($product_accessories)) {
                            foreach ($product_accessories as $product_accessory) {
                                $accessory_cart_product = new HsAccessoryCartProductAbstract($this->context->cart->id, $product_accessory['id_product'], $product_accessory['id_product_attribute'], $product_accessory['id_accessory'], $product_accessory['id_accessory_attribute'], $product_accessory['id_customization']);
    ...
    protected function syncQuantity()
    {
            ...
            if (!empty($cart_products)) {
                foreach ($cart_products as $cart_product) {
                    if ($cart_product['id_product'] == $id_product && $cart_product['id_product_attribute'] == $id_product_attribute) {
                        $id_customization = $cart_product['id_customization'];
                        ...
                        if ($qty_to_check > 0) {
                            $this->context->cart->updateQty($qty_to_check, $id_product, $id_product_attribute, $id_customization, $operation);
                        }
                        break;
                    }
                }
        ...
        $cart_accessories = HsAccessoryCartProductAbstract::getAccessoriesInCart($id_cart, $id_product, $id_product_attribute);
                ...
                foreach ($reload_cart_products as $cart_product) {
                    if ($cart_accessory['id_accessory'] == $cart_product['id_product'] && $cart_accessory['id_accessory_attribute'] == $cart_product['id_product_attribute']) {
                        ...
                        if ($cart_product['id_product'] == (int) Tools::getValue('id_product')) {
                            ...
                            $qty_to_check = $cart_accessory['default_quantity'];
                        }
                        if ($qty_to_check > 0) {
                            $this->context->cart->updateQty($qty_to_check, $cart_product['id_product'], $cart_product['id_product_attribute'], $cart_product['id_customization'], $operation);
                        }
                        $accessory_cart_product = new HsAccessoryCartProductAbstract($id_cart, $cart_accessory['id_product'], $cart_accessory['id_product_attribute'], $cart_accessory['id_accessory'], $cart_accessory['id_accessory_attribute'], $cart_accessory['id_customization']);
    ...
    {
        if (!Configuration::get('HSMA_CHANGE_ACCESSORY_NAME')) {
            return;
        }
        ...
        if (!empty($accessories)) {
            ...
            if (!empty($product_accessories)) {
                if ((int) Configuration::get('HSMA_DEL_ALL_WHEN_DEL_ACCESSORY') === 1) {
                    $main_products = array();
                    foreach ($product_accessories as $product_accessory) {
                        $id_main_products = array('id_product' => $product_accessory['id_product'], 'id_product_attribute' => $product_accessory['id_product_attribute']);
