...
function getCurrentProductOnHomepage(controller = null) {
    let currentProductScript;
    controller = controller !== null ? controller : 'index';
    if (document.querySelector('#current-script[data-controller="' + controller + '"]')) {
        currentProductScript = document.querySelector('#current-script[data-controller="' + controller + '"]');
    } else {
        return false;
    }

    let data = currentProductScript.textContent,
        _products = data.match(new RegExp(/accessoriesTablePrice(\s*=\s*){.*}/, 'g'), '')[0].split(' = '),
        _jsTranslateText = data.match(new RegExp(/jsTranslateText(\s*=\s*){.*}/, 'g'), '')[0].split(' = '),
        _randomMainProductId = data.match(new RegExp(/randomMainProductId(\s*=\s*)([\'|\"]\w+[\'|\"])/, 'g'), '')[0].split(' = '),
        _changeMainPrice = data.match(new RegExp(/changeMainPrice(\s*=\s*)(0|1)/, 'g'), '')[0].split(' = '),
        _subTotal = data.match(new RegExp(/subTotal(\s*=\s*)([\'|\"].*[\'|\"])/, 'g'), '')[0].split(' = '),
        _showTablePrice = data.match(new RegExp(/showTablePrice(\s*=\s*)(0|1)/, 'g'), '')[0].split(' = '),
        _showCombination = data.match(new RegExp(/showCombination(\s*=\s*)(0|1)/, 'g'), '')[0].split(' = '),
        _syncAccessoryQuantity = data.match(new RegExp(/syncAccessoryQuantity(\s*=\s*)(\d)+/, 'g'), '')[0].split(' = '),
        _showOptionImage = data.match(new RegExp(/showOptionImage(\s*=\s*)(0|1)/, 'g'), '')[0].split(' = '),
        _warningOutOfStock = data.match(new RegExp(/warningOutOfStock(\s*=\s*)([\'|\"].*[\'|\"])/, 'g'), '')[0].split(' = '),
        _isScrollingToTablePrice = data.match(new RegExp(/isScrollingToTablePrice(\s*=\s*)(0|1)/, 'g'), '')[0].split(' = '),
        _warningNotEnoughProduct = data.match(new RegExp(/warningNotEnoughProduct(\s*=\s*)([\'|\"].*[\'|\"])/, 'g'), '')[0].split(' = '),
        _warningCustomQuantity = data.match(new RegExp(/warningCustomQuantity(\s*=\s*)([\'|\"].*[\'|\"])/, 'g'), '')[0].split(' = ');

    return {
        products: JSON.parse(_products[1]),
        jsTranslateText: JSON.parse(_jsTranslateText[1]),
        randomMainProductId: _randomMainProductId[1].replace(new RegExp(/[\'|\"]/, 'g'), ''),
        changeMainPrice: parseInt(_changeMainPrice[1]),
        subTotal: _subTotal[1].replace(new RegExp(/[\'|\"]/, 'g'), ''),
        showTablePrice: parseInt(_showTablePrice[1]),
        showCombination: parseInt(_showCombination[1]),
        syncAccessoryQuantity: parseInt(_syncAccessoryQuantity[1]),
        showOptionImage: parseInt(_showOptionImage[1]),
        warningOutOfStock: _warningOutOfStock[1],
        isScrollingToTablePrice: parseInt(_isScrollingToTablePrice[1]),
        warningNotEnoughProduct: _warningNotEnoughProduct[1],
        warningCustomQuantity: _warningCustomQuantity[1]
    };
}

// Remove all keys created with pattern "/(\d)+.(\d)+/"
(function () {
    document.addEventListener("DOMContentLoaded", () => {
        for (let j = 0; j < localStorage.length; j++) {
            if (localStorage.key(j).match(/(\d)+.(\d)+/)) {
...
// Update quantity of main product in Home (Only for last two products)
function updateQuantiyWanted(e, type = "up") {
    const divBootstrapTouchspin = e.target.closest(".input-group.bootstrap-touchspin");
    const idProduct = +divBootstrapTouchspin.closest("form").querySelector("[name='id_product']").value;
    const inputQtyWanted = divBootstrapTouchspin.querySelector("#quantity_wanted");
    const updateQty = (type, qty) => {
        ...
        inputQtyWanted.value = newQty;
    };

    if (divBootstrapTouchspin.querySelectorAll(".input-group-btn").length === 0) {
        localStorage.setItem(e.timeStamp, `${inputQtyWanted.value}_${idProduct}`); // Ex. "40715.67000000505" = 2_3
        if (!localStorage.getItem("timeStampEvent")) localStorage.setItem("timeStampEvent", e.timeStamp);

        // Check whether current timeStamp event is equal to timeStampEvent in localStorage
        // If different, update quantity wanted of current main product
        if (+localStorage.getItem("timeStampEvent") !== e.timeStamp) {
            const _qty = +localStorage.getItem(e.timeStamp).split("_")[0];
            updateQty(type, _qty);
            localStorage.setItem("timeStampEvent", e.timeStamp);
            // console.log(localStorage.getItem("timeStampEvent") + " !== " + e.timeStamp);
        } else {
            // console.log(localStorage.getItem("timeStampEvent") + " === " + e.timeStamp);
        }
    }
...
{
    if (document.getElementById('current-script') && document.getElementById('current-script').dataset.controller === 'index') {
        options = getCurrentProductOnHomepage();
    }
     ...
     * Event load default
     */
    this.onLoad = function (controller = null)
    {
        ...
        if (controller === 'product') {
            $(document).on('click', this._selectors.iconShowBlockGroup, function (event) {
                var element = $(this).find('i');
                PriceTable.instance._onClickBlockGroup(element, event);
            });
        }
        ...
        // up qty button on PS 1.6
        $(document).on('click', this._selectors.classChangeQuantity, function (e) {
            updateQuantiyWanted(e);

            PriceTable.instance.triggerTablePrice();
        });

        // down qty button on PS 1.6
        $(document).on('click', this._selectors.classChangeQuantityDown, function (e) {
            updateQuantiyWanted(e, "down");
     ...
     * @param {Object} element
     */
    this._onClickBlockGroup = function (element, event)
    {
