<div class="blogs blog-type-{$elementWidget.content.view_type} clearfix">
    {if $elementWidget.content.blogs|@count > 0} 
		{$blogLayout = $elementWidget.content.view_type}
		<div class="wrapper-owl">
			<div class="wrapper-owl-item">
				<div class="row-item item-xs-{$elementWidget.content.itemsOwl.xs} item-sm-{$elementWidget.content.itemsOwl.sm} item-md-{$elementWidget.content.itemsOwl.md} item-lg-{$elementWidget.content.itemsOwl.lg} item-xl-{$elementWidget.content.itemsOwl.xl} slider-carousel owl-carousel owl-theme" data-options-owl = "{$elementWidget.content.optionsOwl}">      
					...
					{/if}
					{foreach from=$elementWidget.content.blogs item="post"}
						{if $i mod $y eq 0}
						<div class="item item-{$key}">
						{$key = $key+1}
						{/if}

							{if isset($blogLayout) && $blogLayout}
								{include file="catalog/_partials/miniatures/_partials/_blog/blog-{$blogLayout}.tpl"}
							{/if}
							
						{assign var="i" value="`$i+1`"}
						{if $i mod $y eq 0 || $i eq count($elementWidget.content.blogs)}
						</div>
						{/if}
					{/foreach}
				</div>
    ...
    {else}
        <p class="alert_no_item alert alert-warning">
			{l s='No blogs at this time.' mod='nrtpagebuilder'}
		</p>
    {/if}
</div>   