...
class NrtSlider extends ObjectModel
{
    public $id_nrt_slider;
    public $name;
	public $position;
	// Lang fields
    public $content;
    ...
    public function updatePosition($way, $position)
    {
        if (!$values = Db::getInstance()->executeS('
			SELECT bc.`id_nrt_slider`, bc.`position`
			FROM `'._DB_PREFIX_.'nrt_slider` bc
			ORDER BY bc.`position` ASC'
        ))
            return false;

        foreach ($values as $value){
            if ((int)$value['id_nrt_slider'] == (int)$this->id){
                $moved_menu = $value;
			}
		}
        ...
        return (Db::getInstance()->execute('
					UPDATE `'._DB_PREFIX_.'nrt_slider` bc
            	...
            	&& Db::getInstance()->execute('
					...
					WHERE bc.`id_nrt_slider` = '.(int)$moved_menu['id_nrt_slider']
				)
			);
    }
    ...
    public static function reorderStatic()
    {
		...
		foreach ($result as $value) {
			$return = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'nrt_slider`
			SET `position` = '.(int)$i++.'
			WHERE `id_nrt_slider` = '.(int)$value['id_nrt_slider']);
		}
    ...
    {
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'nrt_slider ORDER BY `position` ASC';
