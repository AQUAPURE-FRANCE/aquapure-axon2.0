    ...
    const NrtMainProductsHome = {
        DOM: {
            ...
            selectors: {
                ...
                iconTouchspinUp: '.material-icons.touchspin-up',
                iconTouchspinDown: '.material-icons.touchspin-down',
        ...
        processActiveSlide(element) {
            this.DOM.activeSlideSelector = `.${element.className.replace(" ", ".")}`;
            let _selectors = this.DOM._selectors;
            for (let j = 0; j < _selectors.length; j++) {
                if (element.querySelectorAll(_selectors[j]).length > 0) {
                    let children = element.querySelectorAll(_selectors[j]);
        ...
        qv.addEventListener('click', Event => {
            ...
            for (let s = 0; s < selectors.length; s++) {
                if (activeElem.querySelectorAll(selectors[s]) !== null) {
                    let elements = activeElem.querySelectorAll(selectors[s]);
                    elements.forEach(el => {
    ...
    document.addEventListener('click', Event => {
        ...
        if (element.classList.contains('quickview') && element.hasAttribute('role') ||
            ...
            for (let s = 0; s < _selectors.length; s++) {
                if (activeElem.querySelectorAll(_selectors[s]) !== null) {
                    let elements = activeElem.querySelectorAll(_selectors[s]);
                    elements.forEach(el => {
                        if (el !== null) {
